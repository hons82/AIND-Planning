...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            for s_level in self.s_levels[level]:
                if s_level in pgnodea.prenodes:
                    pgnodea.parents.add(s_level)
                    s_level.children.add(pgnodea)
                    a_level.add(pgnodea)
    ...
    def add_literal_level(self, level):
        ...
        s_level = set()
        for a_level in self.a_levels[level-1]:
            for eff_node in a_level.effnodes:
                eff_node.parents.add(a_level)
                a_level.children.add(eff_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return len(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) > 0 or \
               len(set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        #
        return len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) > 0 or \
               len(set(node_a1.action.precond_neg) & set(node_a2.action.effect_add)) > 0 or \
               len(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) > 0 or \
               len(set(node_a1.action.precond_pos) & set(node_a2.action.effect_rem)) > 0
